...
if($type=="newuser_happy_mission"||$type=="chk_newuser_mission"||$type=="store_user_mission"){
   ...
   if($type=="store_user_mission"){$stype=trim($_GET['stype']);}
...
if($type=="home_refresh_slider"){
   ...
   if($totalExpPhotos>0){
	  while($exportPhotoRow=mysql_fetch_array($exportPhoto)){
			...
			if(is_file($photoPath)){
				$refreshSlider.='<li><img src="'.$photoPath.'" alt="'.$photoTitle.'" title="'.$photoTitle.'" id="wows1_'.$cnt.'"/></li>';
				$IconRefresh.='<a href="#" title="'.$photoTitle.'"><img src="'.$photoPath.'" alt="'.$photoTitle.'"/>'.$cnt.'</a>';
				$cnt++;
			}	
	   }
   }
   $ajax_data.='<div class="ws_images">
				<ul>'.$refreshSlider.'</ul>
				</div>
