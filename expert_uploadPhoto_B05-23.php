...
if($type=="add"||$type=="del_expert_photo"||$type=="exp_photo_refresh"){
$sort=$_GET['sort'];

 $NewphotoCnt='<div><table><tr><td><a title="Sort" class="btn_small" name="Sort" href="javascript:void(0);" onclick="javascript:ajax_action(\'exp_photo_refresh\',\'right_container\',\'cherryboard_id='.$cherryboard_id.'&sort='.($sort=="asc"?'desc':'asc').'\')">'.($sort=="asc"?'Descending':'Ascending').'</a></td><td><img id="rotate_asc" src="images/transparent.png" height="35" width="35"/></td></tr></table></div>';
				  ...
				  <div class="tag_container">
					<div class="comment_box1" id="photo_title'.$i.'">No Photo</div><div class="clear"></div>
					 ...
					 <div class="clear"></div>
							...
							<div id="div'.$i.'_'.$swap_id.'" ondrop="drop(event,\''.$i.'_'.$swap_id.'\')" ondragover="allowDrop(event,\''.$i.'_'.$swap_id.'\')" class="img_big_container" style="background-image:url('.$photoPath.');cursor:pointer;height:192px;width:192px;padding: 7px;" onclick="javascript:document.getElementById(\'photo_day\').value=\''.$i.'\';document.getElementById(\'photo_upload\').style.display=\'inline\';" src="'.$photoPath.'">
...
if($type=="del_expert_photo"||$type=="exp_photo_refresh"){
