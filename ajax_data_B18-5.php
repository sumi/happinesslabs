...
if($type=="delete_goal_recent_friends"||$type=="sel_goal_recent_friends"||$type=="delete_goal_recent_followers"||$type=="sel_goal_recent_followers"){
	 ...
	 if($meb_id>0){
		...
		$selQuery="select meb_id,req_user_fb_id from ".$tblName." where is_accept='0' and cherryboard_id=".$cherryboard_id." order by meb_id desc limit 10";
		...
		if(mysql_num_rows($selSqlQ)>0){
			...
			while($rowTbl=mysql_fetch_array($selSqlQ)){
				...
				<div class="img_big_container">
					<div class="feedbox_holder">
						<div class="actions"><a class="delete" href="#" onclick="ajax_action(\''.$delTypeName.'\',\''.$divName.'\',\'cherryboard_id='.$cherryboard_id.'&meb_id='.$meb_id.'\')" ><img src="images/delete.png"></a></div>
					</div>
		...
		if($type=="del_cherry_comment"||$type=="del_cherry_expert_comment"){
			...
			$del_sql=mysql_query($del_query);
				...
				$TotalCmt=getFieldValue('count(photo_id)',$tbl_name,'photo_id='.$photo_id);
							...
							if($checkCheers==0){
								$cheersLink='<a href="javascript:void(0);" onclick="add_cherry_cheers(\''.$typeAddCheers.'\',\''.$photo_id.'\',\''.$cherryboard_id.'\',\''.$user_id.'\')" class="red_link_14" style="font-size:12px;">+give cheers!</a>';
							}else{
								if($type=="add_cheers"||$type=="add_expert_cheers"){
									$cheersLink='<span class="fgreen">Cheered!!</span>';
								}
							}
							...
							if($TotalCmt>0){
							  ...
							  while($cmtRow=mysql_fetch_array($selCmt)){
								   ...
								   $photoCnt.='<div class="comment2">
									  ...
									  <strong>'.$UserName.'</strong>&nbsp;&nbsp;'.$PhotoComment.'
