...
if($type=="add"){
   ...
   if($_SERVER['SERVER_NAME']=="localhost"){
	    ...
	    $thumb_command=$ImageMagic_Path."convert ".$old_uploaddir." -thumbnail 195 x  ".$uploaddir;
		$last_line=system($thumb_command, $retval);
   		$thumb_command_thumb=$ImageMagic_Path."convert ".$old_uploaddir." -thumbnail 60 x ".$uploaddirThumb;
...
if($type=="del_photo"||$type=="photo_refresh"){
	 ...
	 for($i=$goal_days;$i>=1;$i--){	
			...
			$photoCnt.='</div>';
							if($TotalCmt>0){
							  ...
							  while($cmtRow=mysql_fetch_array($selCmt)){
								   ...
								   <div class="maryfeed"><div class="action">
								   <a class="delete" href="javascript:void(0);" onclick="add_cherry_comment(event,\'del_cherry_comment\','.$cherryboard_id.','.$photo_id.','.$cmtRow['user_id'].','.$comment_id.')"><img src="images/delete.png" title="Delete"></a></div></div>
								 <div class="leandro_1"><img src="'.$UserPhoto.'" class="img_small" /></div>
							     <div class="leandro_2"><strong>'.$UserName.'</strong>&nbsp;'.$PhotoComment.'
							     </div></div>';
							  }
							}
		...
		$NewphotoCnt='<div style="position: absolute;margin-left:665px;"><table><tr><td><a title="Sort" name="Sort" href="javascript:void(0);" onclick="javascript:ajax_action(\'photo_refresh\',\'right_container\',\'cherryboard_id='.$cherryboard_id.'&sort='.($sort=="asc"?'desc':'asc').'\')">'.($sort=="asc"?'<img title="Desceding" src="images/des.jpg" height="35" width="35"/>':'<img title="Ascending" src="images/asc.jpg" height="35" width="35"/>').'</a></td><td><img id="rotate_asc" src="images/transparent.png" height="35" width="35"/></td></tr></table></div>';
