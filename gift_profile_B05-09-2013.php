...
if(isset($_POST['btnAddExpert'])){
	...
	if($expert_name!=''&&$expert_picture!=''&&$gift_id>0){
	   ...
	   $ischeck=(int)getFieldValue('campaign_expert_id','tbl_app_campaign_experts','campaign_id='.$gift_id);
	   if($ischeck==0){
...
if((int)$_GET['jid']>0){
	...
	while($fetchGiftRow=mysql_fetch_array($sel_gift)){
		...
		//CREATE CHERRYBOARD
		$insRes="INSERT INTO `tbl_app_cherryboard` (`cherryboard_id`, `user_id`, `cherryboard_title`,category_id) VALUES (NULL, '".USER_ID."', '".addslashes($campaign_title)."','".$category_id."')";
		...
		while($selChkRow=mysql_fetch_array($selChk)){
				$insChecklist="INSERT INTO `tbl_app_checklist` (`checklist_id`,user_id, `cherryboard_id`, `checklist`, `record_date`, `is_checked`) VALUES (NULL, '".USER_ID."', '".$cherryboard_id."', '".$selChkRow['checklist']."', CURRENT_TIMESTAMP, '0')";
...
if(isset($_POST['btnSaveCampaign'])&&$_POST['campaign_id']>0){
		...
		if($campaign_title!=""&&$category_id>0&&$goal_days>0&&$sponsor_name!=''&&$sponsor_url!=''){
				 $upd_sel="update tbl_app_gift set campaign_title='".$campaign_title."',campaign_detail='".$campaign_detail."',category_id='".$category_id."',sponsor='".$sponsor."',sponsor_name='".$sponsor_name."', sponsor_url='".$sponsor_url."',miss_days='".$miss_days."',goal_days='".$goal_days."' where gift_id=".$upd_campaign_id;
...
if(isset($_POST['btnAddChk'])){
	...
	for($i=1;$i<=$totalDyndivChk;$i++){
		...
		if($chklist!="Enter checklist"&&$gift_id>0){
			...
			if($chkList==0){
				echo "==>".$insChklist="INSERT INTO `tbl_app_campaign_checklist` (`campaign_chk_id`, `campaign_id`, `checklist`, `record_date`, `is_checked`) VALUES (NULL, '".$gift_id."', '".$chklist."', CURRENT_TIMESTAMP, '0')";
...
<div id="wrapper">
	 <!-- START ADD Reward--- -->
		<?php if($compain_owner_id==USER_ID||USER_ID==26){?>
		<a id="go" rel="leanModal" href="#add_expert1" name="test" style="margin-left:315px;" class="btn_small" title="+ Add Expert">+ Add Expert</a>
		...
		<a href="#upddiv_reward" name="test" class="btn_small">Delete</a>
		<?php } ?>
		<!-- Add Checklist -->
	...
	<div id="div_goal_">
		...
		<?php	
			...
			if($gift_id>0){	
				...
				/*			
				$sel_gift=mysql_query("select * from tbl_app_gift where gift_id=".$gift_id."");
				while($fetchGiftRow=mysql_fetch_array($sel_gift)){
					$campaign_detail=trim($fetchGiftRow['campaign_detail']);
					$campaign_title=str_replace("'","",$fetchGiftRow['campaign_title']);
					$gift_title=ucwords($fetchGiftRow['gift_title']);
					$sponsor_name=ucwords($fetchGiftRow['sponsor_name']);
					$sponsor_url=trim($fetchGiftRow['sponsor_url']);
					$sponsor_logo=$fetchGiftRow['sponsor_logo'];
					$gift_photo=$fetchGiftRow['gift_photo'];
					$num_days=$fetchGiftRow['goal_days'];
					$strikes_days=$fetchGiftRow['miss_days'];
					$campaign_id=(int)$fetchGiftRow['campaign_id'];
					$giftPath='images/gift/'.$gift_photo;
					
					$logoPath='images/gift/'.$sponsor_logo;
					$campaignExpDetail=getFieldsValueArray('expert_name,expert_photo','tbl_app_campaign_experts','campaign_id='.$gift_id);
					$expertName=$campaignExpDetail[0];
					$expertPic=$campaignExpDetail[1];
					$expPicPath='images/gift/'.$expertPic;
	 				...
	 				$fb_post=(int)getFieldValue('fb_post','tbl_app_gift_link','gift_id="'.$gift_id.'" AND user_id='.USER_ID);
					$str_detail='';
					if(strlen($campaign_detail)>100){
						$str_detail=''.substr($campaign_detail,0,100).'...<a href="#" style="text-decoration:none;color:#990000" onclick="ajax_action(\'get_more\',\'div_more\',\'gift_id='.$gift_id.'\')">More</a>';
					}else{
						$str_detail=$campaign_detail;
					}
					
					//find reward joined users
					$selQuery="SELECT b.first_name,b.last_name,b.fb_photo_url FROM tbl_app_cherry_gift a,tbl_app_users b where a.user_id=b.user_id and a.gift_id=".$gift_id." group by a.user_id order by a.record_date";
					...
					if(mysql_num_rows($selGift)>0){
						while($rowGift=mysql_fetch_array($selGift)){
							...
							$UserPhotos.='<div class="gift" style="vertical-align:bottom;padding:0px 3px 0px 0px;"><img src="'.$fb_photo_url.'"  class="imgsmall" title='.$user_name.' style="margin-bottom:0px;" /></div>';
						}	
					}	
					
					if(is_file($giftPath)){
					  $giftCnt.='<table width="950px" align="center" border="0" id="tblgift'.$gift_id.'">
					  			 <tbody>
								 <tr>
								 <td width="239px"><strong>Checklist Items</strong></td>
								 <Td width="300px"><div style="font-size:19px;color:#000000;font-weight:bold">'.ucwords($gift_title).'</div></td>
								 <td width="10px">&nbsp;</td>
								 <td align="right">';
								  if($compain_owner_id==USER_ID||USER_ID==26){
										$giftCnt.='<br/><a id="go" rel="leanModal" href="#upddiv_reward'.$gift_id.'" name="test" class="btn_small">Edit</a>&nbsp;&nbsp;';
							  		}	
					   $giftCnt.='</td>
								 </tr>
								 <tr>
								     <td valign="top"><div id="chk_div">';
									  $selchk=mysql_query("select *,date_format(record_date,'%m-%d-%Y') as record_date from tbl_app_campaign_checklist where campaign_id =".$gift_id." order by campaign_chk_id");
										$checkCnt='';
										$cnt=1;
										while($selchkRow=mysql_fetch_array($selchk)){
					  ...
					  $giftCnt.='</div></td>
								     <td>
										 <img src="'.$giftPath.'" height="200px" width="200px"/></td>
									 <td>&nbsp;</td>
									 <td valign="top">
							...
							'.(trim($str_detail)!=''?''.trim($str_detail).'':'No campaign details').'</div><br/><div class="gift" style="vertical-align:bottom;padding:81px 0px 0px 0px;">Experts:<br/>'.(is_file($expPicPath)?'<img src="'.$expPicPath.'"  class="imgsmall" style="margin-top:4px;" title="'.$expertName.'" />':'').'</div></td>					 						
							<td align="right" valign="top">';
							 if($compain_owner_id==USER_ID||USER_ID==26){
									 $giftCnt.='<br/><a href="gift_graph.php?type=gift&gift_id='.$gift_id.'" name="test" class="btn_small" title="Analytics">Analytics</a>';
							  }		  
							...
							$giftCnt.='
								 ...
								 <tr>
									 ...
									 <td>Time to win reward:<br><font size="+1"><strong>Total
									  '.$num_days.' Days <br> Strikes '.$strikes_days.' Days</strong></font>
									  </td>
								 </tr>
								 ...
								 <tr>
								 	 <td>&nbsp;</td>
								 	 <td><div id="fb_post">
									<img style="cursor:pointer" src="images/'.($fb_post==1?'fb_thanks.png':'fb.jpg').'" width="101px" '.($fb_post==1?'':'onclick="postToFeed(); return false;"').'/>&nbsp;
						...
						if($cherryboard_id==$view_cherryboard_id&&$cherryboard_id>0){
							...
							if($view_cherryboard_id==0){
								$giftCnt.='<a id="join" href="gift_profile.php?jid='.$gift_id.'" name="join" class="btn_small" title="Join">JOIN</a>';
							}
						}					
						$giftCnt.='<br/><br/></td>
								 </tr>
				...
				<script> 
				  ...
				  function postToFeed() {
					...
					var obj = {
					  ...
					  picture: 'http://30daysnew.com/images/gift/<?=$gift_photo?>',
					  name: '<?=ucwords($campaign_title)?>',
			...
			ajax_action('fb_link_post','fb_post','gift_id=<?=$gift_id?>&post_id='+post_id+'&user_id=<?=USER_ID?>');	  
					}
				...
				while($fetchGiftRow=mysql_fetch_array($sel_gift)){
					...
					//END campaign detail
					$campaignExpDetail=getFieldsValueArray('expert_name,expert_photo','tbl_app_campaign_experts','campaign_id='.$campaign_id);
					$expertName=$campaignExpDetail[0];
					$expertPic=$campaignExpDetail[1];
					$expPicPath='images/gift/'.$expertPic;
					...
					if(is_file($gift_path)){
					  ...
					  $giftCnt.='</td>
								     <td>
										 <img src="'.$gift_path.'" height="200px" width="200px"/></td>
									 <td>&nbsp;</td>
									 <td valign="top">
									 <table width="100%">
									 <tr><td>
									 <div id="div_more'.$Div.'">
									 <font size="+1"><strong>'.ucwords($campaign_title).'</strong></font><br>
							'.(trim($str_detail)!=''?''.trim($str_detail).'':'No campaign details').'</div><br/><div class="gift" style="vertical-align:bottom;padding:81px 0px 0px 0px;">Experts:<br/>'.(is_file($expPicPath)?'<img src="'.$expPicPath.'"  class="imgsmall" style="margin-top:4px;" title="'.$expertName.'" />':'').'</div>
 ...
 ?>		
   <input type="hidden" name="campaign_id" id="campaign_id" value="<?=$gift_id?>">
	<div style="display: none; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -330px; top: 200px;" id="upddiv_reward" class="popup_div">
				...
				<div align="center" class="head_20">Edit Campaign</div><br>
    ...
    <textarea id="campaign_detail"  name="campaign_detail" class="search_1" style="height:35px;width:300px;    margin-bottom:5px;vertical-align:top" onFocus="if(this.value=='Enter campaign detail') this.value='';" onBlur="if(this.value=='') this.value='Enter campaign detail';"><?=$campaign_detail?></textarea><br>
         ...
         <input type="text" name="sponsor_name" id="sponsor_name" value="<?=$sponsor_name?>" style="margin-left:21px;margin-bottom:5px;width:170px;"><br>
              ...
              <input type="text" name="sponsor_url" id="sponsor_url" value="<?=$sponsor_url?>" style="margin-left:36px;margin-bottom:5px;"><br>
				 <div class="red_circle">7</div><?=$sponsor_logoPath?><strong>Sponsor logo</strong>&nbsp;:&nbsp;
               ...
               <input type="text" name="goal_days" id="goal_days" value="<?=$goal_days?>" style="margin-bottom:5px;width:50px;margin-left:15px;"><br>
				 <div class="red_circle">9</div><strong>Number of strikes</strong>:
                <input type="text" name="miss_days" id="miss_days" value="<?=$miss_days?>" style="width:50px;margin-bottom:5px;"><br>
				<input type="submit" class="btn_small right" id="btnAddCampaign" value="Save Campaign" name="btnSaveCampaign" />
  </div>
<?php
}
 ?>		
