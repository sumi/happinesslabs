...
if($type=="add_checklist"||$type=="add_expert_checklist"||$type=="remove_checklist"||$type=="remove_expert_checklist"||$type=="checked_checklist"||$type=="checked_expert_checklist"){
	...
	if($user_id==$currentUserId){
		...
		$limit='limit 4';
	 	...
	 	$selchk=mysql_query("select *,date_format(record_date,'%m-%d-%Y') as record_date from ".$tbl_name." where cherryboard_id=".$cherryboard_id." order by checklist_id desc ".$limit."") or die(mysql_error());
		$checkCnt='';
...
if($type=="add_cherry_comment"||$type=="del_cherry_comment"||$type=="add_cheers"||$type=="add_cherry_expert_comment"||$type=="del_cherry_expert_comment"||$type=="add_expert_cheers"||$type=="ask_expert_question"||$type=="cherry_answer"){
	...
	$cherry_comment=addslashes($_GET['cherry_comment']);
		...
		if($type=="ask_expert_question"&&$cherry_question!=""&&$photo_id>0){
			$ins_query="INSERT INTO ".$tbl_name." (`comment_id`, `cherryboard_id`, `photo_id`, `user_id`, `cherry_question`) VALUES (NULL, '".$cherryboard_id."', '".$photo_id."', '".$user_id."', '".$cherry_question."')";
		$ins_sql=mysql_query($ins_query);
		}
		//ADD QUESTION ANSWER 
		if($type=="cherry_answer"&&$cherry_answer!=""&&$photo_id>0){
			$ins_query="INSERT INTO ".$tbl_name." (`comment_id`, `cherryboard_id`, `photo_id`, `user_id`, `cherry_answer`) VALUES (NULL, '".$cherryboard_id."', '".$photo_id."', '".$user_id."', '".$cherry_answer."')";
		$ins_sql=mysql_query($ins_query);
		}
	...
	if($type=="add_expert_cheers"||$type=="add_cherry_expert_comment"||$type=="del_cherry_expert_comment"||$type=="ask_expert_question"||$type=="cherry_answer"){
							  ...
							  while($cmtRow=mysql_fetch_array($selCmt)){
								   ...
								   $photoCnt.='<div class="leandro">
									 ...
									 if($cmtRow['user_id']==$_SESSION['USER_ID']){
									 	$photoCnt.='<div class="action"><a class="delete" href="javascript:void(0);" onclick="add_cherry_comment(event,\''.$typeName.'\','.$cherryboard_id.','.$photo_id.','.$cmtRow['user_id'].','.$comment_id.')"><img src="images/delete.png" title="Delete"></a></div>';
									 }
									 $photoCnt.='</div>
									 <div class="leandro_1"><img src="'.$UserPhoto.'" class="img_small" /></div>
							<div class="leandro_2"><strong>'.$UserName.'</strong>&nbsp;'.$PhotoComment.'</div>
							</div>';				
					if($PhotoQuestion!=''&&$cmtRow['user_id']==26){
			 ...
			 <div class="add_btn"><input type="button" class="btn_small" id="btnanswer" onClick="ajax_action(\'cherry_answer\',\'div_cherry_comment_'.$photo_id.'\',\'photo_id='.$photo_id.'&cherryboard_id='.$cherryboard_id.'&user_id='.$cmtRow['user_id'].'&answer=\'+document.getElementById(\'cherry_answer_'.$photo_id.'\').value);" value="Answer" name="btnanswer" /></div></div>';
					 }
							  }
							}
			$photoCnt.='</div>';
	...
	//START CHERRYBOARD COMMENT AND CHEERS PATRS
		...
		$ajax_data.=$photoCnt;	
		if($type=="ask_expert_question"||$type=="cherry_answer"){
			$ajax_data=$type."##===##".$div_name."##===##".$ajax_data;
	    	echo $ajax_data;
		}else{
...
if($type=="ask_question"){
	 ...
	 $ajax_data.='<div class="add1">
			 ...
			 <div class="add_btn"><input type="button" class="btn_small" id="btnask" onClick="ajax_action(\'ask_expert_question\',\'div_cherry_comment_'.$photo_id.'\',\'photo_id='.$photo_id.'&cherryboard_id='.$cherryboard_id.'&user_id='.$userId.'&question=\'+document.getElementById(\'ask_question_'.$photo_id.'\').value);" value="Ask" name="btnask" /></div>
    </div>';
	 $ajax_data=$type."##===##".$div_name."##===##".$ajax_data;
	 echo $ajax_data;  
}
