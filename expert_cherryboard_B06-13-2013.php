...
if(isset($_POST['photo_day'])&&$_POST['photo_day']>0){
   ...
   $file_size=$_FILES['exp_photo_name']['size'];
...
<?php
 $expertCnt='';
	  ...
	  while($fetchExpertRow=mysql_fetch_array($sel_expert)){
			...
			if($expertPicPath!=""){
				$expertCnt.='<table align="center" border="0" width="85%">';
							if($expertboard_id>0){
...
<a name="fb_share" type="box_count" expr:share_url="data:post.canonicalUrl" href="http://www.facebook.com/sharer.php">Share</a><script src="http://static.ak.fbcdn.net/connect.php/js/FB.Share" type="text/javascript"></script><br/><br/>
								...
								<td valign="top" width="200px">
								   ...
								   <font size="+1"><a '.($expOwner_id==USER_ID?'href="javascript:void(0);"  ondblclick="ajax_action(\'edt_exp_title\',\'div_exp_title_'.$expertboard_id.'\',\'stype=eadd&fieldname=expertboard_title&expertboard_id='.$expertboard_id.'&user_id='.USER_ID.'\')" title="Edit Title"':' href="expert_cherryboard.php?cbid='.$main_BoardId.'"').' class="cleanLink"><strong>'.$expertboard_title.'</strong></a></div><br>
								   <div id="div_more_expert_'.$expertboard_id.'">
								   <a href="javascript:void(0);" '.($expOwner_id==USER_ID?'ondblclick="ajax_action(\'edt_exp_detail\',\'div_more_expert_'.$expertboard_id.'\',\'stype=eadd&fieldname=expertboard_detail&expertboard_id='.$expertboard_id.'&user_id='.USER_ID.'\')"':'').' title="Edit Detail" class="cleanLink">'.(trim($expert_detail)!=''?''.trim($expert_detail).'':'No expert details').'</a>					          
								   </div>
								   <br><div id="div_exp_day_'.$expertboard_id.'"><a href="javascript:void(0);" '.($expOwner_id==USER_ID?'ondblclick="ajax_action(\'edt_exp_goal_day\',\'div_exp_day_'.$expertboard_id.'\',\'stype=eadd&expertboard_id='.$expertboard_id.'&user_id='.USER_ID.'\')"':'').' title="Edit Day" class="cleanLink"><font size="+1"><strong>Total : '.$goal_days.' Days </strong></font></a></div>
								   <div id="div_exp_price_'.$expertboard_id.'"><a href="javascript:void(0);" '.($expOwner_id==USER_ID?'ondblclick="ajax_action(\'edt_exp_price\',\'div_exp_price_'.$expertboard_id.'\',\'stype=eadd&fieldname=price&expertboard_id='.$expertboard_id.'&user_id='.USER_ID.'\')"':'').' title="Edit Price" class="cleanLink"><font size="+1"><strong> Price : '.$price.'</strong></font></a></div><br/>';
								...
								$expertCnt.='</td><td valign="top" width="110px"><font size="+1"><strong>
								Customers </strong></font><br/>';
						...
						if(mysql_num_rows($selExpCustomer)>0){
							while($fetchExpCustomer=mysql_fetch_array($selExpCustomer)){
								...
								$expertCnt.=''.($customerUserId>0?'<img src="'.$customer_photo.'" class="imgsmall" title='.$customer_name.' style="margin-bottom:0px;width:25px;height:25px;" />&nbsp;&nbsp;':'').'';
							}							
						}else{
...
<div id="body_container">
	<div class="wrapper">
		...
		<?php if($expOwner_id==USER_ID){ ?>
		  ...
		  //TO-DO LIST BLOCK
			...
			while($selchkRow=mysql_fetch_array($selchk)){
				...
				$checkCnt.='<div class="box_container" style="width: 230px;"><label><input type="checkbox" id="chkfield_'.$checklist_id.'"  name="chkfield_'.$checklist_id.'" '.($is_checked==1?'checked="checked"':'').' value="1"  onclick="checked_checklist(\'checked_expert_checklist\',\'div_checklist\',\'cherryboard_id='.$cherryboard_id.'&checklist_id='.$checklist_id.'&cuid='.USER_ID.'\',\'chkfield_'.$checklist_id.'\')" class="checkbox"></label>&nbsp;<div id="div_todo_list_'.$checklist_id.'"><a href="javascript:void(0);" '.($chk_user_id==USER_ID?'ondblclick="ajax_action(\'edit_todo_list\',\'div_todo_list_'.$checklist_id.'\',\'stype=eadd&checklist_id='.$checklist_id.'&user_id='.USER_ID.'\')"':'').' title="Edit To-Do List" class="cleanLink">'.$toDoList.'</a></div><span class="smalltext">added '.$record_date.'&nbsp;';
	  ...
	  <div align="center"><table border="0"><tr><td><strong><?=$BuyerName?></strong></td></tr></tr></table></div>
	  <div id="right_container" style="position: absolute;margin-left:275px;width:720px;">
		<div style="position: absolute;margin-left:665px;"><table border="0"><tr><td><a title="Sort" name="Sort" href="javascript:void(0);" onclick="javascript:ajax_action('exp_photo_refresh','right_container','cherryboard_id=<?=$cherryboard_id?>&sort=desc')"><img title="Descending" src="images/des.jpg" height="35" width="35"/></a></td><td><img id="rotate_asc" src="images/transparent.png" height="35" width="35"/></td></table></div>
	  	  <?php
		   ...
		   if(in_array($i,$photoDayArr)){
			...
			while($selphotoRow=mysql_fetch_array($selphoto)){
				...
				if(is_file($photoPath)){
				   $photoCnt.='
				   <table border="0" height="100%" id="divPhoto_'.$photo_id.'">
					<tr>
					<td>
				      <div class="field_container2" style="margin:0px;padding:0px;">
				      <div class="day_container">Day '.$photo_day.'</div>
						  <div class="tag_container">
							<div class="comment_box1" id="photo_title'.$photo_id.'"><a href="javascript:void(0);" '.($user_id==USER_ID?'ondblclick="ajax_action(\'upd_photo_title\',\'photo_title'.$photo_id.'\',\'stype=eadd&photo_id='.$photo_id.'&user_id='.USER_ID.'\')"':'').' title="Edit Comment" class="cleanLink">'.$photo_title.'</a></div><div class="clear"></div>
								<div class="info_box">
									<div id="div_photo_day'.$photo_day.'"><div class="score">
									<a href="javascript:void(0);" '.($user_id==USER_ID?'ondblclick="ajax_action(\'edt_exp_photo_day\',\'div_photo_day'.$photo_day.'\',\'stype=eadd&photo_day='.$photo_day.'&expertboard_id='.$expertboard_id.'&user_id='.USER_ID.'\')"':'').' title="Edit Day Title" style="text-decoration:none;color:#FFFFFF;">'.$DaysTitleArr[$photo_day].'</a></div></div>
