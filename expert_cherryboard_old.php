...
<?php
 $expertCnt='';
	  ...
	  while($fetchExpertRow=mysql_fetch_array($sel_expert)){
			...
			if($expertPicPath!=""){
				$expertCnt.='<table align="center" border="0" width="70%">';
							...
							if($expertboard_id>0){
							  ...
							  if($expOwner_id==USER_ID){
									$expertCnt.='<tr>
										...
										<a href="expert_graph.php?ebid='.$cherryboard_id.'" name="test" class="btn_small" title="Analytics">Analytics</a>
							...
							$expertCnt.='<tr>
								...
								<td valign="top">
								   <div id="div_more">
								   <font size="+1"><a href="expert_cherryboard.php?cbid='.$main_BoardId.'" class="cleanLink"><strong>'.$expertboard_title.'</strong></a></font><br>
						          '.(trim($expert_detail)!=''?''.trim($expert_detail).'':'No expert details').'
								   ...
								   if($check_main_id>0){
										if($created_cherryboard_id>0){
										   ...
										   $expertCnt.='<a href="expertboard.php?eid='.$expertboard_id.'" name="Buy" class="btn_small" title="JOIN">JOIN</a>';
										} 
									}
...
<div id="body_container">
	<div class="wrapper">
        <div id="checklist"><h2>To-Do List</h2>
		<?php if($expOwner_id==USER_ID){ ?>
		  ...
		  //CHECKLIST BLOCK
			 $selchk=mysql_query("select *,date_format(record_date,'%m-%d-%Y') as record_date from tbl_app_expert_checklist where cherryboard_id=".$cherryboard_id." order by checklist_id");
			$checkCnt='';
	  ...
	  <div id="right_container" style="position: absolute;margin-left:275px;width:720px;">
		<div><table border="0"><tr><td><a title="Sort" class="btn_small" name="Sort" href="javascript:void(0);" onclick="javascript:ajax_action('exp_photo_refresh','right_container','cherryboard_id=<?=$cherryboard_id?>&sort=desc')">Descending</a></td><td><img id="rotate_asc" src="images/transparent.png" height="35" width="35"/></td><td><strong><?=$BuyerName?></strong></td></tr></table></div>
	  	  <?php
		   ...
		   if(in_array($i,$photoDayArr)){
			...
			while($selphotoRow=mysql_fetch_array($selphoto)){
				...
				if(is_file($photoPath)){
				   $photoCnt.='<div class="field_container2" id="divPhoto_'.$photo_id.'">
 					...
 					<div class="day_container">Day '.$photo_day.'</div>
						...
						$photoCnt.='<div class="top1">
									<div class="img_big_container3" id="div'.$i.'_'.$swap_id.'" '.($user_id==USER_ID?'ondrop="drop(event,\''.$i.'_'.$swap_id.'\')" ondragover="allowDrop(event,\''.$i.'_'.$swap_id.'\')':'').'"> 
									<div class="feedbox_holder">';
								    if($user_id==USER_ID){
						...
						<div class="bottom1" style="width: 188px; padding: 3px;">';
							...
							if($checkCheers==0){
								$cheersLink='<div class="likes"><a href="javascript:void(0);" onclick="add_cherry_cheers(\'add_expert_cheers\',\''.$photo_id.'\',\''.$cherryboard_id.'\',\''.USER_ID.'\')" class="likes">+ cheers!</a></div>';
							}else{$cheersLink='';}
							$photoCnt.=$cheersLink.'<div class="likes" id="div_ask_que_'.$photo_id.'"><a href="javascript:void(0);" style="padding-left:70px;" onclick="ajax_action(\'ask_question\',\'div_ask_que_'.$photo_id.'\',\'photo_id='.$photo_id.'&cherryboard_id='.$cherryboard_id.'&user_id='.USER_ID.'&day_no='.$photo_day.'\')" class="likes">ask question</a></div><div class="coment" id="div_photo_cheers_'.$photo_id.'">'.(int)$TotalCheers.'&nbsp;Cheers&nbsp;&nbsp;'.(int)$TotalCmt.'&nbsp;Comments&nbsp;';
		...
		$current_userPic=getFieldValue('fb_photo_url','tbl_app_users','user_id='.USER_ID);				
							if($TotalCmt>0){
							  ...
							  while($cmtRow=mysql_fetch_array($selCmt)){
								   ...
								   $PhotoQuestion=stripslashes(trim($cmtRow['cherry_question']));
								   $CherryAnswer=stripslashes(trim($cmtRow['cherry_answer']));
								   if($PhotoComment==''&&$PhotoQuestion!=''){
								   	  $PhotoComment=$PhotoQuestion;
								   }
								   if($PhotoComment==''&&$CherryAnswer!=''){
								   	  $PhotoComment=$CherryAnswer;
								   }						
								    $photoCnt.='<div class="leandro">
									 <div class="maryfeed">';
									if($cmtRow['user_id']==USER_ID){
										 $photoCnt.='<div class="action"><a class="delete" href="javascript:void(0);" onclick="add_cherry_comment(event,\'del_cherry_expert_comment\','.$cherryboard_id.','.$photo_id.','.$cmtRow['user_id'].','.$comment_id.')"><img src="images/delete.png" title="Delete"></a></div>';
									 }
									 $photoCnt.='</div>
									 <div class="leandro_1"><img src="'.$UserPhoto.'" class="img_small" /></div>
									 <div class="leandro_2"><strong>'.$UserName.'</strong>&nbsp;'.$PhotoComment.'</div>
									 </div>';
									 if($PhotoQuestion!=''&&($expOwner_id==USER_ID||getSuperAdmin(USER_ID)==1)){
										$photoCnt.='<div class="add1">
										 <div class="add_img"><img src="'.$current_userPic.'" class="img_small" /></div>
										 <div class="add_txt">
										 <textarea name="cherry_answer_'.$photo_id.'" class="input_comments" id="cherry_answer_'.$photo_id.'" onfocus="if(this.value==\'Add a answer...\') this.value=\'\';" onblur="if(this.value==\'\') this.value=\'Add a answer...\';" style="height: 29px;width:130px;">Add a answer...</textarea>			 
										 </div>
										 <div class="add_btn">
										 <img src="images/btn_answer.png" style="cursor:pointer" onClick="ajax_action(\'cherry_answer\',\'div_cherry_comment_'.$photo_id.'\',\'photo_id='.$photo_id.'&cherryboard_id='.$cherryboard_id.'&user_id='.USER_ID.'&day_no='.$photo_day.'&answer=\'+document.getElementById(\'cherry_answer_'.$photo_id.'\').value);"/>
										 </div>
										 </div>';
									 }
							  }
							...
							/*
							if($expOwner_id==USER_ID){
							  $selCmt=mysql_query("select a.* from tbl_app_expert_cherry_comment a,tbl_app_expert_cherryboard b where a.cherryboard_id=b.cherryboard_id and b.expertboard_id=".$expertboard_id." and a.cherry_comment='' and day_no='".$photo_day."' order by comment_id");
							  while($cmtRow=mysql_fetch_array($selCmt)){
								   $userPhotoArray=getFieldsValueArray('first_name,last_name,fb_photo_url','tbl_app_users','user_id='.$cmtRow['user_id']);
								   $UserName=$userPhotoArray[0].' '.$userPhotoArray[1];
								   $UserPhoto=$userPhotoArray[2];
								   ...
								   if($PhotoComment==''&&$CherryAnswer!=''){
									...
									if($cmtRow['user_id']==USER_ID){
									 ...
									 if($PhotoQuestion!=''&&getSuperAdmin(USER_ID)==1){
										$photoCnt.='<div class="add1">
										 <div class="add_img"><img src="'.$current_userPic.'" class="img_small" /></div>
										 <div class="add_txt">
										 <textarea name="cherry_answer_'.$photo_id.'" class="input_comments" id="cherry_answer_'.$photo_id.'" onfocus="if(this.value==\'Add a answer...\') this.value=\'\';" onblur="if(this.value==\'\') this.value=\'Add a answer...\';" style="height: 29px;width:130px;">Add a answer...</textarea>			 
										 </div>
										 <div class="add_btn"><input type="button" class="btn_small" id="btnanswer" onClick="ajax_action(\'cherry_answer\',\'div_cherry_comment_'.$photo_id.'\',\'photo_id='.$photo_id.'&cherryboard_id='.$cherryboard_id.'&user_id='.USER_ID.'&answer=\'+document.getElementById(\'cherry_answer_'.$photo_id.'\').value);" value="Answer" name="btnanswer" /></div></div>';
									 }
