...
<!--Body Start-->
	<div id="wrapper">
	<div id="div_goal_expert" style="position: absolute;">
	<?php
	// and cherryboard_id=".$cherryboard_id
	$selExpert=mysql_query("select cherryboard_id,user_id,cherryboard_title from tbl_app_expert_cherryboard where record_date>='2013-05-05' order by cherryboard_id");
	...
	if(mysql_num_rows($selExpert)>0){
		while($selExpertRow=mysql_fetch_array($selExpert)){
			...
			$expertCnt.='<div class="field_container1 position_a">
			  <div align="center">
			 ...
			 '.$totalFollowers.' followers<br><br>&nbsp;&nbsp;<a class="blue_btn_small" href="#">&nbsp;Love&nbsp;</a>&nbsp;';
			 $checkBuy=(int)getFieldValue('buy_id','tbl_app_expert_buy','user_id='.USER_ID.' and cherryboard_id='.$cherryboard_id);
	 		 if($checkBuy==0){
			 	$expertCnt.='<a href="#" class="btn_red" id="buy_board_'.$cherryboard_id.'"  onclick="ajax_action(\'buy_board_exp\',\'buy_board_'.$cherryboard_id.'\',\'user_id='.USER_ID.'&cherryboard_id='.$cherryboard_id.'\')">Buy</a>';
			}
			$expertCnt.='<br><br>
		  </div>';
